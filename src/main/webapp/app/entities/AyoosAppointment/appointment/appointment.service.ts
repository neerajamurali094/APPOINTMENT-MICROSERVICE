...
export class AppointmentService {
  ...
  protected convertDateFromClient(appointment: IAppointment): IAppointment {
    const copy: IAppointment = Object.assign({}, appointment, {
      appointmentDateAndTime:
        appointment.appointmentDateAndTime != null && appointment.appointmentDateAndTime.isValid()
          ? appointment.appointmentDateAndTime.toJSON()
          : null
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.appointmentDateAndTime = res.body.appointmentDateAndTime != null ? moment(res.body.appointmentDateAndTime) : null;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((appointment: IAppointment) => {
        appointment.appointmentDateAndTime = appointment.appointmentDateAndTime != null ? moment(appointment.appointmentDateAndTime) : null;
      });
    }
    return res;
  }
