...
/**
 * Performance test for the Timing entity.
 */
class TimingGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Timing entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all timings")
            .get("/ayoosappointment/api/timings")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new timing")
            .post("/ayoosappointment/api/timings")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "day":"2020-01-01T00:00:00.000Z"
                , "startFrom":"2020-01-01T00:00:00.000Z"
                , "endTo":"2020-01-01T00:00:00.000Z"
                }""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_timing_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created timing")
                .get("/ayoosappointment${new_timing_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created timing")
            .delete("/ayoosappointment${new_timing_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
