...
class AppointmentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Appointment entity")
        ...
        .repeat(2) {
            ...
            .get("/ayoosappointment/api/appointments")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new appointment")
            .post("/ayoosappointment/api/appointments")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "trackingId":"SAMPLE_TEXT"
                , "appointmentId":"SAMPLE_TEXT"
                , "chronicDiseaseRef":"SAMPLE_TEXT"
                , "appointmentDateAndTime":"2020-01-01T00:00:00.000Z"
                , "note":"SAMPLE_TEXT"
                , "patientId":"SAMPLE_TEXT"
                , "doctorId":"SAMPLE_TEXT"
                }""")).asJSON
            .check(status.is(201))
            ...
            .repeat(5) {
                ...
                .get("/ayoosappointment${new_appointment_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created appointment")
            .delete("/ayoosappointment${new_appointment_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
