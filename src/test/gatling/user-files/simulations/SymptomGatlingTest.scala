...
class SymptomGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Symptom entity")
        ...
        .repeat(2) {
            ...
            .get("/ayoosappointment/api/symptoms")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new symptom")
            .post("/ayoosappointment/api/symptoms")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "id":null
                , "ref":"SAMPLE_TEXT"
                , "numberOfDaysSuffering":"0"
                }""")).asJSON
            .check(status.is(201))
            ...
            .repeat(5) {
                ...
                .get("/ayoosappointment${new_symptom_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created symptom")
            .delete("/ayoosappointment${new_symptom_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
